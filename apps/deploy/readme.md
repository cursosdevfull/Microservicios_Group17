...
```
eksctl create cluster --name clusterms17 --without-nodegroup --region us-east-1 --zones us-east-1a,us-east-1b,us-east-1c
```

### Crear nodos

```
eksctl create nodegroup --cluster clusterms17 --name clusterms17-nodegroup --node-type t3.medium --nodes 1 --nodes-min 1 --nodes-max 3 --asg-access
```

### Crear IAM Provider

```
eksctl utils associate-iam-oidc-provider --cluster clusterms17 --approve
```
...
```
aws iam create-policy --policy-name AWSLoadBalancerPolicy_clusterms17 --policy-document file://iam_policy.json
```

### Crear la cuenta de servicio para el cluster

```
eksctl create iamserviceaccount --cluster clusterms17 --namespace=kube-system --name=aws-lb-ms-clusterms17 --attach-policy-arn=arn:aws:iam::282865065290:policy/AWSLoadBalancerPolicy_clusterms17 --override-existing-serviceaccounts --approve
```
...
```
helm upgrade -i aws-load-balancer-controller eks/aws-load-balancer-controller --set clusterName=clusterms17 --set serviceAccount.create=false --set serviceAccount.name=aws-lb-ms-clusterms17  -n kube-system
```
...
```
aws eks update-kubeconfig --name clusterdocker16 --region us-east-1
```
